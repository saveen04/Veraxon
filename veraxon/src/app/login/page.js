...
export default function AuthPage() {
  ...
  const redirectUser = (userRole, profileCompleted) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      redirectUser(result.role, result.profileCompleted);
    } catch (err) {
    ...
    try {
      if (isLogin) {
        ...
        if (userDoc.exists()) {
          ...
          // Treat user as onboarded if the flag is set OR if they have institutional data
          const isProfileComplete = !!userData.profileCompleted || !!(userData.collegeName && userData.department);
          redirectUser(userData.role, isProfileComplete);
        } else {
          throw new Error('Institutional profile not found. Attempting manual recovery...');
        }
      } else {
        // Registration Flow
    ...
    <div className="min-h-screen flex selection:bg-[#0052cc] selection:text-white font-inter bg-black overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 text-center px-12">
          <img src="/logov-removebg-preview.png" alt="Veraxon" className="h-32 mb-10 mx-auto drop-shadow-[0_0_50px_rgba(0,82,204,0.6)]" />
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="mb-10">
            ...
            <p className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">
              {isLogin ? 'Authorize access to institutional terminal' : 'Create new verified identity record'}
            </p>
          </div>
          ...
          <div className="flex gap-2 p-1 bg-[#0a0a0a] border border-white/5 rounded-2xl mb-8">
            {['student', 'staff'].map((r) => (
              <button
                key={r}
                onClick={() => setRole(r)}
                className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${role === r ? 'bg-white text-black' : 'text-white/30 hover:bg-white/5'
                  }`}
              >
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-5">
            {!isLogin && (
              <div className="space-y-2">
                <label className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] ml-1">Identity Name</label>
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] ml-1">Access Key (Email)</label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] ml-1">Secure Sequence</label>
              <div className="relative">
            ...
            {!isLogin && (
              <div className="space-y-2">
                <label className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] ml-1">Confirm Sequence</label>
                <input
                  type="password"
            ...
            <div className="flex items-center justify-between px-1">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input
                  ...
                  className="hidden"
                />
                <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${rememberMe ? 'bg-[#0052cc] border-[#0052cc]' : 'border-white/10 group-hover:border-white/30'
                  }`}>
                  {rememberMe && <ShieldCheck size={12} className="text-white" />}
                </div>
                <span className="text-[9px] font-bold text-white/30 uppercase">Persistent Link</span>
              </label>
              {isLogin && (
                <button type="button" className="text-[9px] font-bold text-[#0052cc] uppercase hover:text-white transition-colors">
                  Lost Credentials?
                </button>
              )}
            </div>

            <button type="submit" disabled={loading} className="jira-btn-primary w-full py-4 mt-4 flex items-center justify-center gap-3">
              {loading ? (
                <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
              ) : (
                <>
                  <Fingerprint size={18} />
                  <span>{isLogin ? 'Establish Uplink' : 'Activate Identity'}</span>
                </>
              )}
            </button>
          </form>
          ...
          <div className="my-10 flex items-center gap-4 opacity-5">
            ...
            <span className="text-[8px] font-black text-white uppercase">Secondary Auth</span>
            <div className="h-px flex-1 bg-white" />
          </div>
          ...
          <div className="text-center mt-12">
            ...
            >
              {isLogin ? (
                <>New institutional node? <span className="text-[#0052cc]">Register Unit</span></>
              ) : (
                <>Existing identity? <span className="text-[#0052cc]">Sync Portal</span></>
              )}
            </button>
          </div>
        </div>
