...
export default function LiveProctoringDashboard() {
  ...
  useEffect(() => {
    ...
    // 2. Listen for Infractions / Violations — no orderBy to avoid index requirement
    const qInf = query(collection(db, 'infractions'), limit(50));
    const unsubInf = onSnapshot(qInf, (snapshot) => {
      ...
      console.warn('[proctor] infractions listener:', err.code);
    });
  ...
  return (
    <div className="h-screen bg-[#030303] flex flex-col font-inter text-white overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar role={userData?.role || 'staff'} />
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className={`text-[9px] font-black tracking-widest uppercase mt-1 truncate ${log.severity === 'breach' ? 'text-red-400' : 'text-amber-500/70'}`}>
                                {log.type}
                              </p>
                            </div>
